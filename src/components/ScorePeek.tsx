...
export function ScorePeek({event, onClose, debugHighlight}: {event?: ScoreEvent; onClose: () => void; debugHighlight?: HighlightReviewCandidate}) {
  ...
  return <View style={styles.screen} accessibilityViewIsModal>
      ...
      {!comparisonOpen && !hasBody && __DEV__ && debugHighlight && <View style={styles.debugFacts}>
        <Text style={styles.debugFactsHeading}>HIGHLIGHT REVIEW · SCORE FACTS</Text>
        <Text style={styles.debugFactsText}>No dedicated Smart Score asset exists for m.{debugHighlight.measure}.</Text>
        <Text style={styles.debugFactsText}>Active instruments ({debugHighlight.activeInstruments.length}): {debugHighlight.activeInstruments.join(', ')}</Text>
        <Text style={styles.debugFactsText}>Texture density: {debugHighlight.features.textureDensity}</Text>
        {debugHighlight.hauptstimme.length > 0 && <Text style={styles.debugFactsText}>Hauptstimme: {debugHighlight.hauptstimme.map(span => `${span.part} (${span.label})`).join(', ')}</Text>}
        {debugHighlight.reasons.slice(0, 3).map(reason => <Text key={reason} style={styles.debugFactsText}>• {reason}</Text>)}
      </View>}
      {!comparisonOpen && !hasBody && (!__DEV__ || !debugHighlight) && event && <View style={styles.diagnostic}><Text style={styles.diagnosticText}>Score Peek content is unavailable for m.{event.measure}. Check cue assets and relationship metadata.</Text></View>}
...
const styles = StyleSheet.create({screen: {...StyleSheet.absoluteFillObject, backgroundColor: '#0b111bf5', alignItems: 'center', paddingVertical: 16, overflow: 'hidden'}, panel: {width: '92%', maxWidth: 1180, flex: 1}, header: {fontSize: 22, lineHeight: 28, color: '#f1cd87', letterSpacing: 2}, smartScore: {marginTop: 6, backgroundColor: '#f7f4ed', paddingVertical: 2}, scoreRow: {height: 132, flexDirection: 'row', alignItems: 'center', borderLeftWidth: 5, borderLeftColor: '#687482', paddingRight: 8}, primary: {borderLeftColor: '#f1cd87'}, secondary: {borderLeftColor: '#aeb8c5'}, upcoming: {borderLeftColor: '#78b9d8'}, supporting: {borderLeftColor: '#91a6bd'}, roleLabel: {width: 240, paddingLeft: 16}, instrument: {fontSize: 25, color: '#1b2734', fontWeight: '700'}, role: {fontSize: 18, lineHeight: 23, color: '#354454', marginTop: 3}, scoreImage: {flex: 1, height: 118}, themeLens: {marginTop: 8, padding: 18, backgroundColor: '#f7f4ed', alignItems: 'stretch'}, themeHeadline: {fontSize: 24, lineHeight: 29, color: '#1b2734', fontWeight: '700'}, themeText: {fontSize: 18, lineHeight: 25, color: '#354454', marginTop: 4}, themeImageFrame: {width: '100%', height: 118, marginVertical: 6, alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}, themeImage: {width: '100%', height: '100%'}, comparison: {marginTop: 8, padding: 16, backgroundColor: '#f7f4ed'}, comparisonTitle: {fontSize: 22, color: '#1b2734', fontWeight: '700'}, comparisonCards: {flexDirection: 'row', gap: 16, marginTop: 8}, comparisonCard: {flex: 1}, comparisonLabel: {fontSize: 17, color: '#354454', fontWeight: '700'}, comparisonRole: {fontSize: 17, color: '#354454', marginTop: 2}, comparisonImageFrame: {width: '100%', height: 92, marginTop: 5, alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}, comparisonImage: {width: '100%', height: '100%'}, askPanel: {marginTop: 16, padding: 24, backgroundColor: '#f7f4ed'}, askHeading: {fontSize: 24, color: '#1b2734', fontWeight: '700'}, askText: {fontSize: 22, lineHeight: 31, color: '#354454', marginTop: 14}, askError: {fontSize: 22, lineHeight: 31, color: '#7d2832', marginTop: 14}, diagnostic: {marginTop: 16, padding: 20, backgroundColor: '#351c21'}, diagnosticText: {fontSize: 20, color: '#f4c6c6'}, debugFacts: {marginTop: 16, padding: 20, backgroundColor: '#f7f4ed'}, debugFactsHeading: {fontSize: 22, fontWeight: '700', color: '#1b2734'}, debugFactsText: {fontSize: 18, lineHeight: 25, color: '#354454', marginTop: 4}, footer: {height: 110, marginTop: 'auto', paddingBottom: 28, justifyContent: 'flex-end', flexDirection: 'row', alignItems: 'flex-end'}});
