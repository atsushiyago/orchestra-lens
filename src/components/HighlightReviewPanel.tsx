...
import type {HighlightReviewCandidate, HighlightReviewDecision} from '../playback/highlightReview';
...
export function HighlightReviewPanel({candidate, decision, onDecision, onScore, onExit}: {
  ...
  return <View style={styles.panel}>
    ...
    <View style={styles.actions}>
      <TVButton label="KEEP" onPress={() => onDecision('KEEP')}/>
      <TVButton label="SKIP" onPress={() => onDecision('SKIP')}/>
      <TVButton label="UNREVIEWED" onPress={() => onDecision('UNREVIEWED')}/>
      <TVButton label="SCORE" onPress={onScore}/>
      <TVButton label="EXIT REVIEW" preferred onPress={onExit}/>
    </View>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  actions: {flexDirection: 'row', marginTop: 6, gap: 8, alignItems: 'center'},
});
